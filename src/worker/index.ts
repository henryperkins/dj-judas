...
// ===== MOBILE OPTIMIZATION MIDDLEWARE (2025) =====

// 1. Early Hints (103 Status) - Preload critical resources during "server think time"
// Enables 30% faster page loads by sending resource hints before full response
app.use('*', async (c, next) => {
  const accept = c.req.header('Accept') || '';

  // Only send Early Hints for HTML requests
  if (accept.includes('text/html')) {
    const linkHeaders = [
      // Preload critical CSS (adjusts to content-visibility for mobile)
      '</index.css>; rel=preload; as=style',
      // Preload logo image
      '</assets/logo-7ACBzA0r.jpeg>; rel=preload; as=image',
      // Preconnect to streaming services (Spotify, Apple Music)
      ...
      // Preconnect to R2 if configured
...
// 2. Device-Type Aware Response Headers
// Enables separate caching for mobile, tablet, desktop via Cloudflare's "Cache by Device Type"
app.use('*', async (c, next) => {
  const url = new URL(c.req.url);

  // Set Vary header for device-type caching (configured in Cloudflare Dashboard)
  // This allows Cloudflare to cache different versions for mobile/tablet/desktop
...
app.use('*', async (c, next) => {
  ...
  // Enhanced Security Headers for Mobile Compliance (2025)
  c.header('X-Frame-Options', 'DENY');
  c.header('X-Content-Type-Options', 'nosniff');
  c.header('X-XSS-Protection', '1; mode=block');
  c.header('Referrer-Policy', 'strict-origin-when-cross-origin');
  c.header('Permissions-Policy', 'geolocation=(self), microphone=(self), camera=(self), payment=(self)');

  // Content Security Policy for Mobile Web Standards (2025)
  // Protects against XSS while allowing streaming embeds (Spotify, Apple Music, Facebook, Instagram)
  const cspDirectives = [
    "default-src 'self'",
    // Scripts: Allow self + inline (for React) + streaming SDKs + social SDKs + analytics
    ...
    // Styles: Allow self + inline (for CSS-in-JS and Tailwind)
    "style-src 'self' 'unsafe-inline'",
    // Images: Allow all HTTPS + data URIs + blob for camera/uploads (covers FB/IG CDNs)
    "img-src 'self' https: data: blob:",
    // Fonts: Allow self + data URIs
    "font-src 'self' data:",
    // Connect: API calls to streaming services + social APIs + Cloudflare analytics
    ...
    // Media: Allow all HTTPS + blob for local playback
    "media-src 'self' https: blob:",
    // Frames: Streaming + social embeds
      ...
      if (object) {
        response = new Response(object.body, {
          headers: {
            ...
            // Mobile-optimized caching: 1 year immutable cache for hashed assets
            'Cache-Control': 'public, max-age=31536000, immutable',
            // Enable device-type caching (mobile vs desktop vs tablet)
...
app.get('/events.ics', async (c) => {
  // Disable prefetch for calendar files (Cloudflare Workers don't support speculation)
  const purpose = c.req.header('Sec-Purpose');
  if (purpose === 'prefetch') {
    return new Response(null, { status: 204, headers: { 'Vary': 'Sec-Purpose' } });
  }
...
// Individual event ICS download - using wildcard to handle .ics extension
app.get('/events/*', async (c) => {
  ...
  // Extract slug from path: /events/slug-name.ics -> slug-name
  const slug = path.replace('/events/', '').replace('.ics', '');
  ...
  try {
    ...
    return c.text('Error generating calendar file', 500);
  }
});
...
app.post('/api/admin/gallery', async (c) => {
  ...
  try {
    ...
    // ✅ Fetch the image with automatic optimization via Cloudflare Image Resizing
    // First, validate the URL format
    try {
      new URL(body.url);
    } catch {
      return c.json({ error: 'invalid_url', message: 'Invalid image URL' }, 400);
    }

    // Determine optimal format based on request Accept header
    const accept = c.req.header('Accept') || '';
    ...
    // Fetch with image resizing options
    const upstream = await fetch(body.url, {
      cf: {
        image: {
          width: 1920,          // Max width for web display
          height: 1920,         // Max height (square for flexibility)
          fit: 'scale-down',    // Never enlarge, only shrink if needed
          quality: 85,          // High quality with good compression
          format,               // WebP/AVIF for modern browsers, fallback to JPEG
          metadata: 'none',     // Strip EXIF metadata for privacy & smaller size
          sharpen: 1.0          // Slight sharpening for downscaled images
        }
      }
    });
    ...
    // Validate file size (max 10MB after optimization)
    ...
    const ext = ct.includes('webp') ? 'webp' : ct.includes('avif') ? 'avif' : ct.includes('png') ? 'png' : ct.includes('gif') ? 'gif' : 'jpg';
    const r2Key = `gallery/${id}.${ext}`;

    // ✅ Image Optimization Complete:
    // - Automatically resized to max 1920x1920 (preserves aspect ratio)
    // - Compressed with quality=85 for optimal size/quality balance
    // - Format selection based on browser support (AVIF > WebP > JPEG)
    // - EXIF metadata stripped for privacy and smaller files
    // - Sharpening applied for better downscaled quality
...
app.patch('/api/admin/gallery/reorder', async (c) => {
  ...
  try {
    ...
    // ✅ Use D1 batch API for atomic transaction (all-or-nothing)
    // Prepare a single statement and bind different values for each photo
    const updateStmt = db.prepare(
      `UPDATE gallery_photos SET sort_order = ?, updated_at = ? WHERE id = ?`
    );

    const now = new Date().toISOString();
    const batchStatements = body.order.map(item =>
      updateStmt.bind(item.sort_order, now, item.id)
    );
